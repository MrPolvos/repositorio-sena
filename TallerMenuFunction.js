...
function registroAnimal(idAnimalBd, nombreAnimal, animalIngresado, nombreAnimalngresado) {
...
function tipoCafe(caféConLeche, caféNegro, caféConCrema, caféConHielo) {
...
function voltajeDispositivo(encendido, voltaje) {
    if (encendido) {    
        if (voltaje >= 500) {
...
function saborBebida(bebida, sabor) {
...
function menuTernarias() {
    let ternariaOpcion;
    while(ternariaOpcion !== "6") {
        ternariaOpcion = prompt("Escriba un número para elegir ejercicio con ternarias:\n1. Café con leche\n2. Temperatura café\n3. Crema de café\n4. Verificación ID\n5. Color del café\n6. Volver");
        
        switch(ternariaOpcion) {
            case "1": cafeConLeche(); break;
            case "2": temperaturaCafe(); break;
            case "3": cremaCafe(); break;
            case "4": verificacionID(); break;
            case "5": colorCafe(); break;
...
function menuDecisiones() {
    ...
    while(decisionOpcion !== "7") {
        ...
        switch(decisionOpcion) {
            case "1": escalaPH(12); break;
            case "2": atencionPaciente("Dolor simple"); break;
            case "3": registroAnimal(1234567890, "Max", "1234567890", "Max"); break;
            case "4": tipoCafe(true, false, false, false); break;
            case "5": voltajeDispositivo(true, 500); break;
            case "6": saborBebida("Café Negro", "Amargo"); break;
